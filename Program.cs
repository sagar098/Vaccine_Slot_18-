...
namespace ConsoleApp5
{
    class Program
    {
        ...
        {
            string JsonData = await GetCowinData();
        ...
        public static async Task<String> GetCowinData()
        {
            
            string date = DateTime.Now.ToString("dd-MM-yyyy");
            HttpClient client = new HttpClient();
            string districtCode = ConfigurationSettings.AppSettings.Get("DistrictCode");
            HttpResponseMessage response = await client.GetAsync(String.Format("https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id={1}&date={0}", date, districtCode));
